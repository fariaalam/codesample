<?php  
									...
									if(isset($_GET['add'])){
										...
										mysqli_query($con,"INSERT INTO `junior_officer` (`department`,`name`,`address`,`phone_number`,`appointment_date`,`email`,`password`)
	                                    VALUES ('$department','$name','$address','$phone_number','$appointment_date','$email','$password')");
	                                    }
									}
									
									if($_SESSION['login_type'] == 'senior_officer'){
                                     if(isset($_GET['update'])){
										...
										mysqli_query($con," UPDATE `junior_officer` SET  `performance` =  '$performance' 
		                                                 	
															WHERE  `junior_officer_id` ='$id'");	
	                                    }
									}	
                                  }else{
                                       if(isset($_GET['update'])){
										...
										mysqli_query($con," UPDATE `junior_officer` SET  `department` =  '$department' 
									...
									if(isset($_GET['delete'])){
										...
										mysqli_query($con,"DELETE FROM `junior_officer` WHERE junior_officer_id='$id'");
									}
								?>
<div id="main-content">
		<div class="page-title">
			<div>
				<h1><i class="icon-desktop"></i> Junior Officer</h1>
					...
					<div class="box-content">
						<div class="tab-content">
											...
											?>
												<tr>
													...
													<?php 
														...
														$_SESSION['login_type'] == 'managing_director') {
													?>
													<td><a href="index.php?page=junior_officer&edit=<?php echo $row['junior_officer_id']; ?>" >Edit</a></td>
													<?php } ?>
													<?php 
														if($_SESSION['login_type'] == 'admin' ||
                                                           $_SESSION['login_type'] == 'managing_director') {
													?>
													<td><a href="index.php?page=junior_officer&delete=<?php echo $row['junior_officer_id']; ?>" onclick="return confirm('Are you sure to delete this?')" >Delete</a></td>
													<?php }?>
							...
							<div class="tab-pane" id="add">
								...
								<div class="box-content">
									<form action="index.php?page=junior_officer&add=do" class="form-horizontal" method="post">
							...
							<?php if(isset($_GET['edit'])){ 
							   $jun= $_GET['edit'];
							   $result = mysqli_query($con,"SELECT * FROM junior_officer WHERE junior_officer_id='$jun'");
							   while($row = mysqli_fetch_array($result)){
							?>
							<div class="tab-pane active" id="edit">
								<form action="index.php?page=junior_officer&update=do" method="post" class="form-horizontal">
								     <?php 
														if($_SESSION['login_type'] == 'admin' ||
                                                           $_SESSION['login_type'] == 'managing_director') {
													?>
										...
										<input type="hidden" name="id" value='<?php echo $jun; ?>' />
