<?php  
									if(isset($_GET['add'])){
										...
										mysqli_query($con,"INSERT INTO `senior_officer` (`department`,`name`,`address`,`phone_number`,`appointment_date`,`email`,`password`)
	                                    VALUES ('$department','$name','$address','$phone_number','$appointment_date','$email','$password')");
	                                    }
									}
									if($_SESSION['login_type'] == 'executive_officer'){
                                     if(isset($_GET['update'])){
										...
										mysqli_query($con," UPDATE `senior_officer` SET  `performance` =  '$performance'
		                                                 	
															WHERE  `senior_officer_id` ='$id'");	
	                                    }
									}
                                   }else{
                                          if(isset($_GET['update'])){
										...
										mysqli_query($con," UPDATE `senior_officer` SET  `department` =  '$department' 
									...
									if(isset($_GET['delete'])){
										...
										mysqli_query($con,"DELETE FROM `senior_officer` WHERE senior_officer_id='$id'");
									}
								?>
<div id="main-content">
		<div class="page-title">
			<div>
				<h1><i class="icon-desktop"></i> Senior Officer</h1>
					...
					<div class="box-content">
						<div class="tab-content">
												...
												<tr>
													...
													<?php 
														if($_SESSION['login_type'] == 'admin' ||
														   ...
														   $_SESSION['login_type'] == 'executive_officer') {
													?>
													<td><a href="index.php?page=senior_officer&edit=<?php echo $row['senior_officer_id']; ?>" >Edit</a></td>
													<?php }?>
													<?php 
														if($_SESSION['login_type'] == 'admin' ||
                                                           $_SESSION['login_type'] == 'managing_director') {
													?>
													<td><a href="index.php?page=senior_officer&delete=<?php echo $row['senior_officer_id']; ?>" onclick="return confirm('Are you sure to delete this?')" >Delete</a></td>
													<?php } ?>
												</tr>
											  <?php	}?>
											<!--php loop end -->
							...
							<div class="tab-pane" id="add">
								...
								<div class="box-content">
									<form action="index.php?page=senior_officer&add=do" class="form-horizontal" method="post">
							...
							<?php if(isset($_GET['edit'])){ 
							   $sen = $_GET['edit'];
							   $result = mysqli_query($con,"SELECT * FROM senior_officer WHERE senior_officer_id='$sen'");
							   while($row = mysqli_fetch_array($result)){
							?>
							
								
									<div class="tab-pane active" id="edit">
								<form action="index.php?page=senior_officer&update=do" method="post" class="form-horizontal">
								    <?php 
														if($_SESSION['login_type'] == 'admin' ||
                                                           $_SESSION['login_type'] == 'managing_director'
														   ) {
									...
									<!--	<div class="control-group">
										...
										<div class="control-group">
    										...
    										<input type="hidden" name="id" value='<?php echo $sen; ?>' />
