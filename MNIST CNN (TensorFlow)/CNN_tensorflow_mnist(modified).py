...
conv3= conv_layer(input=relulayer2,
                   num_input_channels=num_filters2,
                   filter_size=filter_size3,
                   num_filters=num_filters3)

pool3=pooling_layer(conv3);
relulayer3 = relu_layer(pool3);

flat_layer, num_features = flat_layer(relulayer3);
...
fc_layer2_relu=relu_layer(fc_layer2);

fc_layer3 = fc_layer(input=fc_layer2_relu,
                         num_inputs=512,
                         num_outputs=64)

fc_layer3_relu=relu_layer(fc_layer3);

fc_layer4= fc_layer(input=fc_layer3_relu,
                         num_inputs=64,
...
cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=fc_layer4,
                                                        labels=y_true)
